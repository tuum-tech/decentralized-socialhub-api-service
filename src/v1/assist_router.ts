...
assistRouter.post('/didtx/create', async (req, res) => {
    ...
    console.info("Executing: /v1/assist_router/didtx/create");

    const url = `${process.env.ASSIST_API_URL}/v1/didtx/create`;
...
assistRouter.get('/didtx/confirmation_id/:confirmation_id', async (req, res) => {
     ...
     // tslint:disable-next-line:no-console
     console.info("Executing: /v1/assist_router/didtx/confirmation_id/{confirmation_id}");
...
assistRouter.get('/didtx/did/:did', async (req, res) => {
    ...
    // tslint:disable-next-line:no-console
    console.info("Executing: /v1/assist_router/didtx/did/{did}");

    const url = `${process.env.ASSIST_API_URL}/v1/didtx/did/${req.params.did}`;
    const ret: any = await handleRoute(url, req.body, getAssistHeader(), false);
...
assistRouter.get('/didtx/recent/did/:did', async (req, res) => {
    ...
    const url = `${process.env.ASSIST_API_URL}/v1/didtx/recent/did/${req.params.did}`;
    const ret: any = await handleRoute(url, req.body, getAssistHeader(), false);
...
assistRouter.get('/documents/did/:did', async (req, res) => {
    ...
    const url = `${process.env.ASSIST_API_URL}/v1/documents/did/${req.params.did}`;
    const ret: any = await handleRoute(url, req.body, getAssistHeader(), false);

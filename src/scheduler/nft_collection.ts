...
const getAssetsUsingElacityAPI = async (address: string) => {
...
const getAssetsUsingMoralisAPI = async (address: string, chain: string) => {
  let cursor = "";
  let assets: any[] = [];
  do {
      ...
      await response.response.get_nft_collection_spaces.items.map(
        async (space: any) => {
          ...
          if (network.toLowerCase() === "elastos smart contract chain") {
            assets = await getAssetsUsingElacityAPI(address);
          } else {
            // use Opensea API
            ...
            // Else use Moralis API
              ...
              assets = await getAssetsUsingMoralisAPI(address, chain);
            }
          }
          ...
          if (assets.length > 0) {
            ...
            // tslint:disable-next-line:no-console
            console.log(`all *${space.name}* retrieved.`);
          }
        }
      )
    );
